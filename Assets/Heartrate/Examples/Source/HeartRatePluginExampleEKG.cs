...
public class HeartRatePluginExampleEKG : MonoBehaviour {
...
//	private	bool			b_ControlPointFeatured = false;
	...
	private int[]			i_EKGArray = {0,2,4,5,4,2,0,0,0,-6,48,-10,0,0,0,0,0,2+2/3,5+1/3,8,9,8,5+1/3,2+2/3,0}; 
	...
	void Update() {
		if (m_state == MenuState.CONNECTED) {
			if (m_tickrate <= 0) {
				...
				for (int i = 0; i < i_EKGArray.Length; i++) {
					temp.Add(new Vector3(i*.1f,i_EKGArray[i]*.1f,1));
				}
